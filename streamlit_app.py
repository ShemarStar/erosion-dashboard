...
if fetch_data:
    try:
        ...
        # Fetch road network (cached for speed)
        @st.cache_data
        def load_graph(region):
            return ox.graph_from_place(region, network_type="all")
        graph = load_graph(region)
        ...
        # Fetch water bodies (cached)
        @st.cache_data
        def load_water(region):
            water_tags = {"natural": "water", "waterway": True}
            return ox.features_from_place(region, tags=water_tags)
        water_gdf = load_water(region)
